 ...
 *
 * \param[in] mem_device->device_read A function pointer which is used to read from the disk.
 * \param[in] mem_device->device_read_interval A function pointer which is used to read in constant intervals from the disk.
 ...
 * \see partition_close
 */
struct partition_struct* partition_open(MEM_Device* mem_device, int8_t index)
{
    struct partition_struct* new_partition = 0;
    uint8_t buffer[0x10];

    if(!mem_device->device_read || !mem_device->device_read_interval || index >= 4)
        return 0;

    if(index >= 0)
    {
        /* read specified partition table index */
        if(!mem_device->device_read(mem_device->context, 0x01be + index * 0x10, buffer, sizeof(buffer)))
            return 0;
    ...
    new_partition->device_read = mem_device->device_read;
    new_partition->device_read_interval = mem_device->device_read_interval;
    new_partition->device_write = mem_device->device_write;
    new_partition->device_write_interval = mem_device->device_write_interval;
    ...
    new_partition->context = mem_device->context;
