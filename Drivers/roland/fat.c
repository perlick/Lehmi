...
uint8_t fat_read_header(struct fat_fs_struct* fs)
{
    ...
    if(!partition->device_read(partition->context, partition_offset + 0x0b, buffer, sizeof(buffer)))
        return 0;
...
cluster_t fat_get_next_cluster(const struct fat_fs_struct* fs, cluster_t cluster_num)
{
    ...
    if(fs->partition->type == PARTITION_TYPE_FAT32)
    {
        ...
        uint32_t fat_entry;
        if(!fs->partition->device_read(fs->partition->context, fs->header.fat_offset + (offset_t) cluster_num * sizeof(fat_entry), (uint8_t*) &fat_entry, sizeof(fat_entry)))
            return 0;
...
cluster_t fat_append_clusters(struct fat_fs_struct* fs, cluster_t cluster_num, cluster_t count)
{
    ...
    void* context = fs->partition->context;
    ...
    for(cluster_t cluster_left = cluster_count; cluster_left > 0; --cluster_left, ++cluster_current)
    {
        ...
        if(is_fat32)
        {
            if(!device_read(context, fat_offset + (offset_t) cluster_current * sizeof(fat_entry32), (uint8_t*) &fat_entry32, sizeof(fat_entry32)))
                return 0;
        }
        else
#endif
        {
            if(!device_read(context, fat_offset + (offset_t) cluster_current * sizeof(fat_entry16), (uint8_t*) &fat_entry16, sizeof(fat_entry16)))
                return 0;
        }
        ...
        if(is_fat32)
        {
            ...
            if(!device_write(context, fat_offset + (offset_t) cluster_current * sizeof(fat_entry32), (uint8_t*) &fat_entry32, sizeof(fat_entry32)))
                break;
        }
        else
#endif
        {
            ...
            if(!device_write(context, fat_offset + (offset_t) cluster_current * sizeof(fat_entry16), (uint8_t*) &fat_entry16, sizeof(fat_entry16)))
                break;
        }
    ...
    do
    {
        ...
        if(cluster_num >= 2)
        {
            ...
            if(is_fat32)
            {
                ...
                if(!device_write(context, fat_offset + (offset_t) cluster_num * sizeof(fat_entry32), (uint8_t*) &fat_entry32, sizeof(fat_entry32)))
                    break;
            }
            else
#endif
            {
                ...
                if(!device_write(context, fat_offset + (offset_t) cluster_num * sizeof(fat_entry16), (uint8_t*) &fat_entry16, sizeof(fat_entry16)))
                    break;
            }
        }
...
uint8_t fat_free_clusters(struct fat_fs_struct* fs, cluster_t cluster_num)
{
    ...
    if(fs->partition->type == PARTITION_TYPE_FAT32)
    {
        ...
        while(cluster_num)
        {
            if(!fs->partition->device_read(fs->partition->context, fat_offset + (offset_t) cluster_num * sizeof(fat_entry), (uint8_t*) &fat_entry, sizeof(fat_entry)))
                return 0;
            ...
            fs->partition->device_write(fs->partition->context, fat_offset + (offset_t) cluster_num * sizeof(fat_entry), (uint8_t*) &fat_entry, sizeof(fat_entry));
...
uint8_t fat_terminate_clusters(struct fat_fs_struct* fs, cluster_t cluster_num)
{
    ...
    if(fs->partition->type == PARTITION_TYPE_FAT32)
    {
        ...
        if(!fs->partition->device_write(fs->partition->context, fs->header.fat_offset + (offset_t) cluster_num * sizeof(fat_entry), (uint8_t*) &fat_entry, sizeof(fat_entry)))
            return 0;
    }
    else
#endif
...
uint8_t fat_clear_cluster(const struct fat_fs_struct* fs, cluster_t cluster_num)
{
    ...
    return fs->partition->device_write_interval(fs->partition->context,
                                                cluster_offset,
                                                zero,
...
intptr_t fat_read_file(struct fat_file_struct* fd, uint8_t* buffer, uintptr_t buffer_len)
{
    ...
    do
    {
        ...
        /* read data */
        if(!fd->fs->partition->device_read(fd->fs->partition->context, cluster_offset, buffer, copy_length))
...
intptr_t fat_write_file(struct fat_file_struct* fd, const uint8_t* buffer, uintptr_t buffer_len)
{
    ...
    do
    {
        ...
        if(!fd->fs->partition->device_write(fd->fs->partition->context, cluster_offset, buffer, write_length))
            break;
...
uint8_t fat_read_dir(struct fat_dir_struct* dd, struct fat_dir_entry_struct* dir_entry)
{
    ...
    while(!arg.finished)
    {
        ...
        arg.bytes_read = 0;
        if(!fs->partition->device_read_interval(fs->partition->context,
                                                pos,
                                                buffer,
                                                sizeof(buffer),
                                                cluster_left,
...
offset_t fat_find_offset_for_dir_entry(struct fat_fs_struct* fs, const struct fat_dir_struct* parent, const struct fat_dir_entry_struct* dir_entry)
{
    ...
    while(1)
    {
        ...
        uint8_t first_char;
        if(!fs->partition->device_read(fs->partition->context, offset, &first_char, sizeof(first_char)))
            return 0;
...
uint8_t fat_write_dir_entry(const struct fat_fs_struct* fs, struct fat_dir_entry_struct* dir_entry)
{
    ...
    /* write to disk */
#if FAT_LFN_SUPPORT
    if(!device_write(context, offset + (uint16_t) lfn_entry_count * 32, buffer, sizeof(buffer)))
#else
    ...
    for(uint8_t lfn_entry = lfn_entry_count; lfn_entry > 0; --lfn_entry)
    {
        ...
        /* write entry */
        device_write(context, offset, buffer, sizeof(buffer));
...
uint8_t fat_delete_file(struct fat_fs_struct* fs, struct fat_dir_entry_struct* dir_entry)
{
    ...
    while(1)
    {
        ...
        if(!fs->partition->device_read(fs->partition->context, dir_entry_offset, buffer, sizeof(buffer)))
            return 0;
        ...
        /* write back entry */
        if(!fs->partition->device_write(fs->partition->context, dir_entry_offset, buffer, sizeof(buffer)))
            return 0;
    ...
    if(!fs->partition->device_write(fs->partition->context, dir_entry_offset, &first_char, 1))
        return 0;
#endif
